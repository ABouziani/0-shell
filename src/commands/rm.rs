...
// pub fn rm(args: &Vec<&str>) -> Result<(), Box<dyn Erorr>> {
//     if args.len() <= 1 {
//         return Err("No files or directories provided.".into());
//     }

//     let is_recursive = args.containe(&"-r");
//     let paths: Vec<_> = args.iter().skip(1).filter(|&&arg| arg != "-r").collect();
//     if paths.is_empty() {
//         return Err("No files or directories provided.".into());
//     }
//     for path in paths {
//         remove_file_or_dir(path, is_recursive)?;
//     }
//     Ok(())
// }


// fn remove_file_or_dir(path: &str, is_recursive: bool) -> Result<(), Box<dyn Erorr>> {
...
// fn remove_directory_recursive(dir: &Path) -> Result<(), Box<dyn Erorr>> {
